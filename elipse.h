...
#include"oblects.h"
template <class T>
class Ellips :public objects<T>
{
public:
	Ellips();
	Ellips(Line<T> a, objects<T> b);
	...
	objects<T> GetOsX();
	objects<T> GetOsY();
	void SetOsX(objects<T> a);
	void SetOsY(objects<T> b);
...
template<class T>
inline Ellips<T>::Ellips(objects<T> a, objects<T> b)
{
	A = a;
	B = b;
}
...
{
	vyvod << "Ýëëèïñ ñ îñÿìè a= " << (*this).A.Length() << ", b= " << (*this).B.Length();
	return vyvod;
}
...
template<class T>
inline objects<T> Ellips<T>::GetOsY()
{
	return B;
}
